...
function Resizerpanel(props) {
  ...
  function percentageToUnit(percentage, originalSize, unit) {
    ...
    switch (unit) {
      ...
      case "in":
        return Math.round(pixels / 73); // 73 PPI
      case "cm":
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                <input
                  ...
                  value={crop.width}
                  onChange={(e) => {
                    const newSize = Math.max(1, Math.min(100, parseInt(e.target.value, 10) || 1)); // Ensure size is between 1 and 100

                    // Calculate the difference in size
                    const sizeDifference = newSize - crop.width;

                    // Adjust the position to resize in place
                    const newX = Math.max(0, Math.min(100 - newSize, crop.x - sizeDifference / 2));
                    const newY = Math.max(0, Math.min(100 - newSize, crop.y - sizeDifference / 2));
